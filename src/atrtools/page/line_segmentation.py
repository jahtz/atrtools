...
class LineSegmentation:
    ...
    def process(self, img: PathLike | np.ndarray, xml: PathLike | PageXML) -> PageXML:
        ...
        Returns:
            The input PageXML object containing the computed TextLines
        """
    ...
    def __process_region(self, img: np.ndarray, region: PageElement) -> None:
        region_polygon: np.ndarray | None = pageu.element_to_polygon(region)
        ...
        region_bin = np.array(clipped_img <= imageu.midrange(clipped_img), bool)
        ...
        for llabel, poly, baseline in line_polys:
            ...
            lelement.create(PageType.Coords, points=pageu.polygon_to_points(line_polygon))
            ...
            if baseline:
                ...
                lelement.create(PageType.Baseline, points=pageu.polygon_to_points(line_baseline))
    ...
    def __compute_gradmaps(
        ...
        threshold = (0.5, 4)
        boxmap = imageu.compute_boxmap(img, scale, threshold=threshold)
        cleaned = boxmap * img
        ...
        bottom = npu.norm_max((grad < 0) * (-grad))
        top = npu.norm_max((grad > 0) * grad)
    ...
    def __compute_line_seeds(
        ...
        if robust:
            bmarked = maximum_filter(bmarked, (1, npu.odd(scale))) * (1 - colseps)
        tmarked = maximum_filter(tmarked, (1, npu.odd(scale))) * (1 - colseps)
        ...
        for x in range(bmarked.shape[1]):
            ...
            transitions = sorted(
                [(y, 1) for y in npu.find(bmarked[:, x])]
                + [(y, 0) for y in npu.find(tmarked[:, x])]
            )[::-1]
            if robust:
                ln = 0
        ...
        if robust:
            ...
            seeds = morphu.rb_opening(seeds, (npu.odd(scale / 2), npu.odd(scale)))
        else:
            ...
            seeds = maximum_filter(seeds, (1, npu.odd(1 +  scale)))
        # interrupt by column separators before labelling:
        seeds = seeds * (1 - colseps)
        seeds, nlabels = morphu.label(seeds)
        return seeds
    ...
    def __hmerge_line_seeds(
        ...
        for label in labels:
            ...
            seed = morphu.rb_closing(seed, (scale, scale))
            if not seed.any():
                continue
            ...
            for label2 in labels:
                ...
                for y in shared_y:
                    ...
                    if can_x_max < new_x_min:
                        if seps is None or not seps[y, can_x_max:new_x_min].any():
                            gap[y, can_x_max:new_x_min] = True
                    if new_x_max < can_x_min:
                        if seps is None or not seps[y, new_x_max:can_x_min].any():
    ...
    def __compute_baselines(
        ...
        baselabels, nbaselabels = morphu.label(blines)
        baseslices = [(slice(0, 0), slice(0, 0))] + morphu.find_objects(baselabels)
        
        # if multiple labels per seed, ignore the ones above others (can happen due to mis-estimation of scale)
        corrs = morphu.correspondences(linelabels, baselabels).T
        labelmap = {}
        for line in np.unique(linelabels):
            ...
            for i, indi in enumerate(corrinds[:-1]):
                ...
                for j, indj in enumerate(corrinds[i + 1 :], i + 1):
                    ...
                    if slu.xoverlaps(baseslicei, baseslicej):
    ...
    def __compute_segmentation(
        ...
        bottom, top, boxmap = self.__compute_gradmaps(img, scale, False)
        ...
        llabels = morphu.propagate_labels_majority(img, seeds)
        llabels2 = morphu.propagate_labels(img, seeds, conflict=0)
        ...
        llabels2 = morphu.propagate_labels_simple(img, seeds2)
        ...
        spread = morphu.spread_labels(llabels, maxdist=spread_dist or scale / 2)
        llabels2 = morphu.propagate_labels_majority(img, spread)
        llabels = np.where(seeds, seeds, llabels2)
        llabels[sepmask > 0] = seplabel
        llabels = morphu.spread_labels(llabels, maxdist=spread_dist or scale / 2)
        llabels[llabels == seplabel] = 0

        logger.debug('Sorting labels by reading order')
        try:
            llabels = morphu.reading_order(llabels, rl, bt)[llabels]
