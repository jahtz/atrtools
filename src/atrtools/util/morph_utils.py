...
class morphu:
    ...
    def select_regions(img: np.ndarray, f, min: float = 0, nbest: int = 100000) -> np.ndarray:
        ...
        objects = morphu.find_objects(labels)
    ...
    def rb_opening(img, size: tuple[int, int], origin: int = 0) -> np.ndarray:
        ...
        """
        return cv2.morphologyEx(img.astype(np.uint8), cv2.MORPH_OPEN, morphu._brick(size))
    ...
    def rb_closing(img: np.ndarray, size: tuple[int, int], origin: int = 0) -> np.ndarray:
        ...
        """
        return cv2.morphologyEx(img.astype(np.uint8), cv2.MORPH_CLOSE, morphu._brick(size))
    ...
    def r_dilation(img: np.ndarray, size: tuple[int, int], origin: int = 0) -> np.ndarray:
        ...
        """
        return cv2.dilate(img.astype(np.uint8), morphu._brick(size))
    ...
    def propagate_labels_simple(regions: np.ndarray, labels: np.ndarray) -> np.ndarray:
        ...
        """
        rlabels, _ = morphu.label(regions)
        cors = morphu.correspondences(rlabels, labels, False)
    ...
    def propagate_labels_majority(img: np.ndarray, labels: np.ndarray) -> np.ndarray:
        ...
        Returns:
            Labeled image with propagated labels.
        """
        rlabels, _ = morphu.label(img)
        cors = morphu.correspondences(rlabels, labels)
        outputs = np.zeros(np.amax(rlabels) + 1, 'i')
    ...
    @staticmethod
    def spread_labels(labels: np.ndarray, maxdist: int | float = 9999999) -> np.ndarray:
        """
    ...
    def reading_order(seg: np.ndarray, rl: bool = False, bt: bool = False) -> np.ndarray:
        ...
        objects = [(slice(0, 0), slice(0, 0))] + morphu.find_objects(seg)
        ...
        for i, j in zip(yorder[:-1], yorder[1:]):
            ...
            if (
                oi 
                and oj 
                and slu.yoverlaps(oi, oj) 
                and (slu.ycenter_in(oi, oj) or slu.ycenter_in(oj, oi)) 
                and not any([
                    slu.xoverlaps(oj, objects[k]) and slu.xoverlap_rel(oj, objects[k]) > 0.1 
                    for k in groups[-1]
                ])
            ):
                groups[-1].append(j)
        ...
        for group in groups:
            ...
            xs = pos(slu.xcenter, [objects[i] for i in group])
