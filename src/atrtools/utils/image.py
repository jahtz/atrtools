...
class ImageUtils:
    ...
    def binary_objects(img: np.ndarray) -> list[tuple[slice, ...]]:
        ...
        return MorphUtils.find_objects(labels)
    ...
    def compute_boxmap(
        ...
        """
        objects = ImageUtils.binary_objects(img)
        bysize = sorted(objects, key=SliceUtils.area)
        boxmap = np.zeros(img.shape, dtype)
        for o in reversed(bysize):
            if SliceUtils.area(o) ** 0.5 < threshold[0] * scale:  # only too small boxes (noise) from here on
                break
            if SliceUtils.area(o) ** 0.5 > threshold[1] * scale:  # ignore too large box
                continue
            boxmap[o] = 1
        return boxmap
